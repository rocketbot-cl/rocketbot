...
// Humanize time
!function(){var n={en:{y:function(n){return"year"+(1===n?"":"s")},mo:function(n){return"month"+(1===n?"":"s")},w:function(n){return"week"+(1===n?"":"s")},d:function(n){return"day"+(1===n?"":"s")},h:function(n){return"hour"+(1===n?"":"s")},m:function(n){return"minute"+(1===n?"":"s")},s:function(n){return"second"+(1===n?"":"s")},ms:function(n){return"millisecond"+(1===n?"":"s")},decimal:"."},es:{y:function(n){return"año"+(1===n?"":"s")},mo:function(n){return"mes"+(1===n?"":"es")},w:function(n){return"semana"+(1===n?"":"s")},d:function(n){return"día"+(1===n?"":"s")},h:function(n){return"hora"+(1===n?"":"s")},m:function(n){return"minuto"+(1===n?"":"s")},s:function(n){return"segundo"+(1===n?"":"s")},ms:function(n){return"milisegundo"+(1===n?"":"s")},decimal:","}};function t(t){var e=function(t,a){return function(t,e){var u,a,s;t=Math.abs(t);var l,c,f,m=function(t){var e=[t.language];if(o(t,"fallbacks")){if(!i(t.fallbacks)||!t.fallbacks.length)throw new Error("fallbacks must be an array with at least one element");e=e.concat(t.fallbacks)}for(var r=0;r<e.length;r++){var u=e[r];if(o(t.languages,u))return t.languages[u];if(o(n,u))return n[u]}throw new Error("No language found.")}(e),g=[];for(u=0,a=e.units.length;u<a;u++){if(l=e.units[u],c=e.unitMeasures[l],u+1===a)if(o(e,"maxDecimalPoints")){var h=Math.pow(10,e.maxDecimalPoints),d=t/c;f=parseFloat((Math.floor(h*d)/h).toFixed(e.maxDecimalPoints))}else f=t/c;else f=Math.floor(t/c);g.push({unitCount:f,unitName:l}),t-=f*c}var p,v,y=0;for(u=0;u<g.length;u++)if(g[u].unitCount){y=u;break}if(e.round)for(u=g.length-1;u>=0&&((s=g[u]).unitCount=Math.round(s.unitCount),0!==u);u--)v=g[u-1],p=e.unitMeasures[v.unitName]/e.unitMeasures[s.unitName],(s.unitCount%p==0||e.largest&&e.largest-1<u-y)&&(v.unitCount+=s.unitCount/p,s.unitCount=0);var w=[];for(u=0,g.length;u<a&&((s=g[u]).unitCount&&w.push(r(s.unitCount,s.unitName,m,e)),w.length!==e.largest);u++);if(!w.length)return r(0,e.units[e.units.length-1],m,e);if(!e.conjunction||1===w.length)return w.join(e.delimiter);if(2===w.length)return w.join(e.conjunction);if(w.length>2)return w.slice(0,-1).join(e.delimiter)+(e.serialComma?",":"")+e.conjunction+w.slice(-1)}(t,u({},e,a||{}))};return u(e,{language:"en",delimiter:", ",spacer:" ",conjunction:"",serialComma:!0,units:["y","mo","w","d","h","m","s"],languages:{},round:!1,unitMeasures:{y:315576e5,mo:26298e5,w:6048e5,d:864e5,h:36e5,m:6e4,s:1e3,ms:1}},t)}var e=t({});function r(n,t,e,r){var u;u=o(r,"decimal")?r.decimal:o(e,"decimal")?e.decimal:".";var i,a=n.toString().replace(".",u),s=e[t];return i="function"==typeof s?s(n):s,a+r.spacer+i}function u(n){for(var t,e=1;e<arguments.length;e++)for(var r in t=arguments[e])o(t,r)&&(n[r]=t[r]);return n}var i=Array.isArray||function(n){return"[object Array]"===Object.prototype.toString.call(n)};function o(n,t){return Object.prototype.hasOwnProperty.call(n,t)}e.getSupportedLanguages=function(){var t=[];for(var e in n)o(n,e)&&"gr"!==e&&t.push(e);return t},e.humanizer=t,"function"==typeof define&&define.amd?define(function(){return e}):"undefined"!=typeof module&&module.exports?module.exports=e:this.humanizeDuration=e}();

angular.module('rocketbotApp',[])
.controller('MainController', function($timeout, $rootScope, $scope, $http){
   
    $scope.data = data;
    $scope.file_name = "Export"
    
    $scope.file_folder = ""
    $scope.viewLogicals= false;
    $scope.viewDescription= false;
    $scope.viewData= false;
    $scope.viewScreenshot= false;   
    $scope.commands = JSON.parse(data[0].data);
    $rootScope.robot_name = $scope.commands.project.profile.name;
    $scope.tree =commands.commands;
    $scope.item_viewing = {}
    $scope.item_screenshot = {}
    $scope.item_comands = []
    $scope.modules = modules
    $scope.exportToDocx = function(typedoc){
        $scope.loading_docx = true;
        $http.post('../toDocx', 
        $.param({
            'db': db,
            'views': JSON.stringify($scope.item_screenshot),
            'viewLogicals': $scope.viewLogicals,
            'viewDescription': $scope.viewDescription,
            'viewData': $scope.viewData,
            'viewScreenshot': $scope.viewScreenshot,
            'robot':  $rootScope.robot_name,
            'typedoc': typedoc
        }))
        .then(function(data){
            $scope.loading_docx = false;

        })
        console.log($scope.item_screenshot)
    }
    ...
    $scope.getFatherData = function (parent) {
        ...
        var gi = function (params) {
            ...
            for (var t = 0; t < params.length; t++) {
                ...
                if (params[t].else && params[t].else.length > 0) {
    ...
    $scope.viewBot = function(bot){
        var url = window.location.origin + "/treeview/" + bot + "/"+db;
        window.open(url)
        console.log(url)
    }
...
.filter('humanize', function () {
    return function (value) {
      try {
        return humanizeDuration(value*1000, { maxDecimalPoints: 2 })
      } catch (error) {
        return '';
      }
