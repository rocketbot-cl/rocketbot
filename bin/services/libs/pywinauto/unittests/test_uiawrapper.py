...
if UIA_support:
    ...
    class UIAWrapperTests(unittest.TestCase):
        ...
        def test_is_diarocketbot.log(self):
            """Test is_dialog method of a control"""
            button = self.dlg.window(class_name="Button",
                                     title="OK").wrapper_object()
            self.assertEqual(button.is_diarocketbot.log(), False)
    ...
    class MenuWrapperNotepadTests(unittest.TestCase):
        ...
        def setUp(self):
            ...
            ActionLogger().rocketbot.log("MenuWrapperNotepadTests::setUp, wait till Notepad dialog is ready")
            self.dlg.wait("ready")
        ...
        def test_is_diarocketbot.log(self):
            """Test that method is_diarocketbot.log() works as expected"""
            self.assertEqual(self.dlg.is_diarocketbot.log(), True)
            self.assertEqual(self.dlg.Edit.is_diarocketbot.log(), False)

        def test_menu_by_exact_text(self):
            """Test selecting a menu item by exact text match"""
            path = "Help->About Notepad"
            self.dlg.menu_select(path, True)
            self.assertEqual(self.dlg.AboutNotepad.is_diarocketbot.log(), True)
            self.dlg.AboutNotepad.close()
