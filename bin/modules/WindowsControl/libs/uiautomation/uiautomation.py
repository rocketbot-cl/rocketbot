...
class Control():
    ...
    def Exists2(self, maxSearchSeconds: float = 5, searchIntervalSeconds: float = SEARCH_INTERVAL, printIfNotExist: bool = False) -> bool:
        """
        maxSearchSeconds: float
        searchIntervalSeconds: float
        Find control every searchIntervalSeconds seconds in maxSearchSeconds seconds.
        ...
        while True:
            control = FindControl(self.searchFromControl, self._CompareFunction, 1, False, self.foundIndex)
            if control:
                self._element = control.Element
                control._element = 0  # control will be destroyed, but the element needs to be stroed in self._element
                return True
    ...
    def ExistsWindow(self, maxSearchSeconds: float = 5, searchIntervalSeconds: float = SEARCH_INTERVAL, printIfNotExist: bool = False) -> bool:
        """
        maxSearchSeconds: float
        searchIntervalSeconds: float
        Find control every searchIntervalSeconds seconds in maxSearchSeconds seconds.
    ...
    def Disappears(self, maxSearchSeconds: float = 5, searchIntervalSeconds: float = SEARCH_INTERVAL,
        ...
        start = ProcessTime()
        while True:
            exist = self.Exists(float(5), searchIntervalSeconds)
            if not exist:
                return True
            else:
    ...
    def draw_outline(self,colour = 'green',thickness = 2,fill = 1,rect = None):
        ...
        hPen = ctypes.windll.gdi32.CreatePen(0, thickness, colour)

        # create the brush (inside)
        brush = win32structures.LOGBRUSH()
        brush.lbStyle = fill
        brush.lbHatch = 5
        hBrush = ctypes.windll.gdi32.CreateBrushIndirect(ctypes.byref(brush))

        # get the Device Context
        dc = ctypes.windll.gdi32.CreateDCW(u"DISPLAY", None, None, None )

        # push our objects into it
        ctypes.windll.gdi32.SelectObject(dc, hBrush)
        ctypes.windll.gdi32.SelectObject(dc, hPen)
