...
class OtherTests(unittest.TestCase):
    ...
    def test_writing_from_existing_zinfo(self):
        """Using a zinfo to initialise a new file to be written creates new ZipInfo."""
        with zipfile.ZipFile(TESTFN, 'w') as zipfp:
            # Create a file
            with zipfp.open('test.txt', 'w') as fout:
                fout.write(b'hi')

            # Use the ZipInfo object of the file we just created to open a new
            # file to be written
            zinfo = zipfp.filelist[0]
            with zipfp.open(zinfo, 'w') as fout:
                fout.write(b'bye')

            # The two files that are now in the zip archive should have
            # different ZipInfo instances. If they do not, we lose the ability
            # to read from the first one. This is a different behaviour to
            # opening a file with a duplicate filename which allows access to
            # both file entries via indexing zipfp.filelist.
            self.assertNotEqual(
                zipfp.filelist[0].header_offset,
                zipfp.filelist[1].header_offset
            )
            self.assertNotEqual(zipfp.filelist[0], zipfp.filelist[1])
